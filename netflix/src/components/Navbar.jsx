...
const Navbar = () => {
        ...
        const fetchMovies = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    setFiltered(response.data.movies);
                } else {
    ...
    const handleLogout = async () => {
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'https://cinema-flame-seven.vercel.app/api/'}user/logout`, {
                method: 'POST',
                credentials: 'include',
            });
    ...
    return (
        <div className="sticky top-0 z-50 bg-black/60 backdrop-blur-md flex flex-col md:flex-row md:items-center md:justify-evenly p-4 gap-2">
            ...
            <div className="hidden md:flex flex-wrap justify-center gap-2 text-sm text-yellow-400 font-semibold">
                ...
                <button
                    onClick={handleLogout}
                    className="flex items-center gap-1 px-4 py-2 bg-red-600 hover:bg-red-700 rounded-md text-sm text-white"
                >
                ...
                <a
                    href="/Sbskripsn"
                    className="relative px-6 py-3 rounded-[15px] bg-[#f3ff07] text-[#212121] font-extrabold text-lg shadow-[4px_8px_19px_-3px_rgba(0,0,0,0.27)] transition-all duration-300 overflow-hidden 
                    before:content-[''] before:absolute before:top-0 before:left-0 before:h-full before:w-0 before:rounded-[15px] before:bg-[#0df4ec] before:-z-10 before:shadow-[4px_8px_19px_-3px_rgba(0,0,0,0.27)] before:transition-all before:duration-300 hover:text-[#e8e8e8] hover:before:w-full"
                >
                    Subscribe
                </a>
            </div>
            ...
            {/* Search Overlay */}
            {showOverlay && (
                <div className="fixed top-[7rem]  inset-0 bg-black/60 z-[999] flex justify-center items-center py-24">
                    <div
                        ref={overlayRef}
                        className="bg-white py-24 rounded-lg h-[80vh] w-full max-w-4xl overflow-y-auto shadow-lg"
                    >
