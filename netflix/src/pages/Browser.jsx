...
const Browser = ({ movieId }) => {
  ...
  useEffect(() => {
    const fetchUpComingMovies = async () => {
      try {
        const response = await axios.get('https://cinemo-pearl.vercel.app/api/show/upcomming-movies');
        if (response.data.success) {
  ...
  useEffect(() => {
    const fetchTrailer = async () => {
      try {
        const response = await axios.get(`https://cinemo-pearl.vercel.app/api/show/movie/${movieId}`);
        ...
        <div className="glass relative w-full lg:h-[30rem] lg:w-[55rem] rounded-2xl overflow-hidden lg:ml-20">
          <div className="aspect-video w-full">
            <iframe
              ...
              src={`${trailerUrl.replace('watch?v=', 'embed/')}?autoplay=1&loop=1&playlist=${trailerUrl.split('v=')[1]}&mute=1&controls=0&rel=0&modestbranding=1&enablejsapi=1`}
              title="Movie Trailer"
              frameBorder="0"
